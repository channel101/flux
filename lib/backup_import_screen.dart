...
import 'package:flux/backup_service.dart';
import 'package:flux/habit.dart';
import 'package:flux/storage_service.dart';
import 'package:flux/data_service.dart';
...
class _BackupImportScreenState extends State<BackupImportScreen> {
  ...
  Future<void> _createBackup() async {
    try {
      ...
      if (success) {
        ...
        await _loadData(); // Refresh the backup list
      }
    } catch (e) {
      Navigator.of(context).pop(); // Close loading dialog
  ...
  Future<void> _importBackup() async {
    try {
      _showLoadingDialog('Importing backup...');
      
      final result = await BackupService.importBackup();
      
      Navigator.of(context).pop(); // Close loading dialog
      
      if (result.success && result.hasHabits) {
        _showImportDialog(result);
      } else {
        _showErrorSnackBar(result.errors.isNotEmpty 
            ? result.errors.first 
            : 'No valid habits found in backup');
      }
    } catch (e) {
      Navigator.of(context).pop(); // Close loading dialog
